...
const Sidebar: React.FC<SidebarProps> = ({ role }) => {
  ...
  return (
    <div className={cn(
      ...
      {/* Header */}
      <div className="p-4 border-b border-line-light dark:border-line-dark">
        <div className="flex items-center justify-between">
          {!isCollapsed && (
            <h2 className="font-display text-lg font-semibold text-neutral-900 dark:text-white capitalize">
              {role} Dashboard
            </h2>
          )}
          <button
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="p-1 rounded-md hover:bg-neutral-100 dark:hover:bg-jade-400/10 transition-colors"
          >
            {isCollapsed ? (
